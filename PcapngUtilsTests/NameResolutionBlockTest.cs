...
namespace PcapngUtilsTests
{
    ...
    public class NameResolutionBlockTest
    {
        [Theory]
        [InlineData(true)]
        [InlineData(false)]
        public void NameResolutionBlock_ConvertToByte_Test(bool reorder)
        {
            ...
            using (MemoryStream stream = new MemoryStream(byteblock))
            {
                using (BinaryReader binaryReader = new BinaryReader(stream))
                {
                    ...
                    Assert.Equal(prePacketBlock.NameResolutionRecords.Count, postPacketBlock.NameResolutionRecords.Count);
                    for (int i = 0; i < prePacketBlock.NameResolutionRecords.Count; i++)
                    {
                        Assert.Equal(prePacketBlock.NameResolutionRecords[i], postPacketBlock.NameResolutionRecords[i]);
                    }
                    Assert.Equal(prePacketBlock.Options.Comment, postPacketBlock.Options.Comment);
                    Assert.Equal(prePacketBlock.Options.DnsName, postPacketBlock.Options.DnsName);
                    Assert.Equal(prePacketBlock.Options.DnsIp4Addr, postPacketBlock.Options.DnsIp4Addr);
                    Assert.Equal(prePacketBlock.Options.DnsIp6Addr, postPacketBlock.Options.DnsIp6Addr);
                }
            }
        }
    }
